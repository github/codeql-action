...
/**
 * Fetch the given remote branch with shallow exclusion. Errors are ignored.
 */
const fetchShallowExclude = async function (branch, exclude) {
    try {
        await runGitCommand((0, exports.getOptionalInput)("checkout_path"), [
            ...
            "origin",
            `${branch}:${branch}`,
        ], `Cannot fetch ${branch} excluding ${exclude}.`);
    }
    catch {
        // Ignore errors
    }
};
exports.fetchShallowExclude = fetchShallowExclude;
/**
 ...
 */
const getAllMergeBases = async function (refs) {
    try {
...
exports.getAllMergeBases = getAllMergeBases;
/**
...
const getGitDiffHunkHeaders = async function (fromRef, toRef) {
    let stdout = "";
    try {
        stdout = await runGitCommand((0, exports.getOptionalInput)("checkout_path"), ["diff", "--no-renames", "--irreversible-delete", "-U0", fromRef, toRef], `Cannot get diff from ${fromRef} to ${toRef}.`);
    }
    catch {
        return undefined;
    }
