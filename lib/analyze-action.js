...
async function run() {
    ...
    try {
        ...
        const automationId = util.getAutomationID(await actionsUtil.getAnalysisKey(), actionsUtil.getRequiredInput("matrix"));
        const queriesStats = await analyze_1.runAnalyze(outputDir, util.getMemoryFlag(actionsUtil.getOptionalInput("ram")), util.getAddSnippetsFlag(actionsUtil.getRequiredInput("add-snippets")), util.getThreadsFlag(actionsUtil.getOptionalInput("threads"), logger), automationId, config, logger);
