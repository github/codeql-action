...
async function runQueries(sarifFolder, memoryFlag, addSnippetsFlag, threadsFlag, automationDetailsId, config, logger) {
    ...
    for (const language of config.languages) {
        ...
        try {
            ...
            if (queries["builtin"].length > 0) {
                ...
                querySuitePaths.push(await runQueryGroup(language, "builtin", queries["builtin"], undefined));
            ...
            for (let i = 0; i < queries["custom"].length; ++i) {
                if (queries["custom"][i].queries.length > 0) {
                    querySuitePaths.push(await runQueryGroup(language, `custom-${i}`, queries["custom"][i].queries, queries["custom"][i].searchPath));
                    ranCustom = true;
                }
            }
            if (ranCustom) {
    ...
    async function runQueryGroup(language, type, queries, searchPath) {
        const databasePath = util.getCodeQLDatabasePath(config, language);
