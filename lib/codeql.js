    ...
    const codeql = {
        ...
        async databaseInitCluster(config, sourceRoot, processName, qlconfigFile, logger) {
            ...
            if (overlayDatabaseEnv === "overlay") {
                (0, assert_1.default)(config.buildMode === util_1.BuildMode.None);
            ...
            const overwriteFlags = overlayDatabaseEnv === "overlay"
                ? []
                : (0, tools_features_1.isSupportedToolsFeature)(await this.getVersion(), tools_features_1.ToolsFeature.ForceOverwrite)
                    ? ["--force-overwrite"]
                    : ["--overwrite"];
            await runCli(cmd, [
                "database",
            ...
            const baseFileOidsPath = path.join(config.dbLocation, "git-file-oids.json");
            if (overlayDatabaseEnv === "overlay") {
                const baseFileOids = JSON.parse(await fs.promises.readFile(baseFileOidsPath, "utf-8"));
                const headFileOids = await (0, git_utils_1.getAllFileOids)(sourceRoot);
                const changes = compareFileOids(baseFileOids, headFileOids);
                const repoRoot = await (0, git_utils_1.getGitRoot)(sourceRoot);
        ...
        async databaseRunQueries(databasePath, flags) {
            const overlayDatabaseEnv = process.env["CODEQL_OVERLAY_DATABASE_INIT"];
            const cacheFlags = overlayDatabaseEnv === "overlay-base"
                ? []
...
function compareFileOids(baseFileOids, headFileOids) {
    const changes = [];
