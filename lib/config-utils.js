 ...
 */
async function addRemoteQueries(codeQL, resultMap, queryUses, tempDir, githubUrl, githubAuth, logger, configFile) {
    ...
    const checkoutPath = await externalQueries.checkoutExternalRepository(nwo, ref, githubUrl, githubAuth, tempDir, logger);
 ...
 */
async function parseQueryUses(languages, codeQL, resultMap, queryUses, tempDir, checkoutPath, githubUrl, githubAuth, logger, configFile) {
    ...
    await addRemoteQueries(codeQL, resultMap, queryUses, tempDir, githubUrl, githubAuth, logger, configFile);
}
...
async function addQueriesFromWorkflow(codeQL, queriesInput, languages, resultMap, tempDir, checkoutPath, githubUrl, githubAuth, logger) {
    queriesInput = queriesInput.trim();
    // "+" means "don't override config file" - see shouldAddConfigFileQueries
    queriesInput = queriesInput.replace(/^\+/, "");
    for (const query of queriesInput.split(",")) {
        await parseQueryUses(languages, codeQL, resultMap, query, tempDir, checkoutPath, githubUrl, githubAuth, logger);
    }
}
 ...
 */
async function getDefaultConfig(languagesInput, queriesInput, repository, tempDir, toolCacheDir, codeQL, checkoutPath, githubAuth, githubUrl, externalRepositoryToken, logger) {
    const languages = await getLanguages(languagesInput, repository, githubAuth, githubUrl, logger);
    const queries = {};
    await addDefaultQueries(codeQL, languages, queries);
    if (queriesInput) {
        await addQueriesFromWorkflow(codeQL, queriesInput, languages, queries, tempDir, checkoutPath, githubUrl, (externalRepositoryToken !== null && externalRepositoryToken !== void 0 ? externalRepositoryToken : githubAuth), logger);
    }
    return {
        languages,
        queries,
 ...
 */
async function loadConfig(languagesInput, queriesInput, configFile, externalRepositoryToken, repository, tempDir, toolCacheDir, codeQL, checkoutPath, githubAuth, githubUrl, logger) {
    let parsedYAML;
    if (isLocal(configFile)) {
        ...
        parsedYAML = await getRemoteConfig(configFile, (externalRepositoryToken !== null && externalRepositoryToken !== void 0 ? externalRepositoryToken : githubAuth), githubUrl);
    }
    ...
    if (shouldAddConfigFileQueries(queriesInput) &&
        ...
        for (const query of parsedYAML[QUERIES_PROPERTY]) {
            ...
            await parseQueryUses(languages, codeQL, queries, query[QUERIES_USES_PROPERTY], tempDir, checkoutPath, githubUrl, (externalRepositoryToken !== null && externalRepositoryToken !== void 0 ? externalRepositoryToken : githubAuth), logger, configFile);
        }
    }
 ...
 */
async function initConfig(languagesInput, queriesInput, configFile, externalRepositoryToken, repository, tempDir, toolCacheDir, codeQL, checkoutPath, githubAuth, githubUrl, logger) {
    let config;
    // If no config file was provided create an empty one
    if (!configFile) {
        logger.debug("No configuration file was provided");
        config = await getDefaultConfig(languagesInput, queriesInput, repository, tempDir, toolCacheDir, codeQL, checkoutPath, githubAuth, githubUrl, externalRepositoryToken, logger);
    }
    else {
        config = await loadConfig(languagesInput, queriesInput, configFile, externalRepositoryToken, repository, tempDir, toolCacheDir, codeQL, checkoutPath, githubAuth, githubUrl, logger);
    }
