 ...
 */
async function getDefaultConfig({ languagesInput, queriesInput, packsInput, buildModeInput, dbLocation, trapCachingEnabled, dependencyCachingEnabled, debugMode, debugArtifactName, debugDatabaseName, repository, tempDir, codeql, sourceRoot, githubVersion, features, logger, }) {
    ...
    const augmentationProperties = await calculateAugmentation(codeql, features, packsInput, queriesInput, languages, sourceRoot, buildMode, logger);
 ...
 */
async function loadConfig({ languagesInput, queriesInput, packsInput, buildModeInput, configFile, dbLocation, trapCachingEnabled, dependencyCachingEnabled, debugMode, debugArtifactName, debugDatabaseName, repository, tempDir, codeql, workspacePath, sourceRoot, githubVersion, apiDetails, features, logger, }) {
    let parsedYAML;
...
async function calculateAugmentation(codeql, features, rawPacksInput, rawQueriesInput, languages, sourceRoot, buildMode, logger) {
    ...
    const { overlayDatabaseMode, useOverlayDatabaseCaching } = await getOverlayDatabaseMode(codeql, features, sourceRoot, buildMode, logger);
 ...
 */
async function getOverlayDatabaseMode(codeql, features, sourceRoot, buildMode, logger) {
    ...
    else if (await features.getValue(feature_flags_1.Feature.OverlayAnalysis, codeql)) {
