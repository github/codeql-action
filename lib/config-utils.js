...
async function calculateAugmentation(codeql, repository, features, rawPacksInput, rawQueriesInput, rawQualityQueriesInput, languages, sourceRoot, buildMode, logger) {
    ...
    const { overlayDatabaseMode, useOverlayDatabaseCaching } = await getOverlayDatabaseMode(codeql, repository, features, sourceRoot, buildMode, logger);
 ...
 */
async function getOverlayDatabaseMode(codeql, repository, features, sourceRoot, buildMode, logger) {
    ...
    else if (["github", "dsp-testing"].includes(repository.owner) &&
