...
async function getSupportedLanguageMap(codeql, features, logger) {
    const resolveSupportedLanguagesUsingCli = await features.getValue(feature_flags_1.Feature.ResolveSupportedLanguagesUsingCli, codeql);
    const resolveResult = await codeql.betterResolveLanguages({
        filterToLanguagesWithQueries: resolveSupportedLanguagesUsingCli,
    });
    if (resolveSupportedLanguagesUsingCli) {
        logger.debug(`The CodeQL CLI supports the following languages: ${Object.keys(resolveResult.extractors).join(", ")}`);
    }
    ...
    for (const extractor of Object.keys(resolveResult.extractors)) {
        ...
        // a known language.
        if (resolveSupportedLanguagesUsingCli ||
            languages_1.KnownLanguage[extractor] !== undefined) {
...
async function getDefaultConfig({ languagesInput, queriesInput, qualityQueriesInput, packsInput, buildModeInput, dbLocation, trapCachingEnabled, dependencyCachingEnabled, debugMode, debugArtifactName, debugDatabaseName, repository, tempDir, codeql, sourceRoot, githubVersion, features, logger, }) {
    const languages = await getLanguages(codeql, languagesInput, repository, sourceRoot, features, logger);
