...
ava_1.default("No detected languages", async (t) => {
    mockListLanguages([]);
    try {
        await configUtils.getLanguages(undefined, { owner: 'github', repo: 'example ' }, 'token', 'https://github.example.com', logging_1.getRunnerLogger(true));
        throw new Error('initConfig did not throw error');
    }
    catch (err) {
...
ava_1.default("Unknown languages", async (t) => {
    ...
    try {
        await configUtils.getLanguages(languages, { owner: 'github', repo: 'example ' }, 'token', 'https://github.example.com', logging_1.getRunnerLogger(true));
        throw new Error('initConfig did not throw error');
    }
    catch (err) {
