...
[
    ...
    for (const resolveSupportedLanguagesUsingCli of [true, false]) {
        (0, ava_1.default)(`getLanguages${resolveSupportedLanguagesUsingCli ? " (supported languages via CLI)" : ""}: ${args.name}`, async (t) => {
            const features = (0, testing_utils_1.createFeatures)(resolveSupportedLanguagesUsingCli
                ? [feature_flags_1.Feature.ResolveSupportedLanguagesUsingCli]
                : []);
            ...
            const codeQL = (0, codeql_1.createStubCodeQL)({
                betterResolveLanguages: (options) => Promise.resolve({
                    aliases: {
            ...
            if (args.expectedLanguages) {
                // happy path
                const actualLanguages = await configUtils.getLanguages(codeQL, args.languagesInput, mockRepositoryNwo, ".", features, mockLogger);
                t.deepEqual(actualLanguages.sort(), args.expectedLanguages.sort());
            }
            else {
                // there is an error
                await t.throwsAsync(async () => await configUtils.getLanguages(codeQL, args.languagesInput, mockRepositoryNwo, ".", features, mockLogger), { message: args.expectedError });
            }
