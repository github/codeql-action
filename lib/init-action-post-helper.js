...
async function maybeUploadFailedSarif(config, repositoryNwo, featureEnablement, logger) {
    ...
    const existingDatabasePaths = (0, util_1.getExistingDatabasePaths)(config);
    if (existingDatabasePaths.length === 0) {
        ...
        const uploadStatusReports = [];
        const maybeIsDatabaseCluster = process.env[shared_environment_1.CODEQL_ACTION_IS_DATABASE_CLUSTER];
        ...
        existingDatabasePaths.map(async (databasePath) => {
            ...
            await codeql.databaseExportDiagnostics(databasePath, maybeIsDatabaseCluster === undefined ||
                maybeIsDatabaseCluster.length === 0
                ? false
                : true, databaseSarifFile, category);
            core.info(`Uploading failed SARIF file ${databaseSarifFile}`);
            const uploadResult = await uploadLib.uploadFromActions(databaseSarifFile, checkoutPath, category, logger);
            await uploadLib.waitForProcessing(repositoryNwo, uploadResult.sarifID, logger, { isUnsuccessfulExecution: true });
...
async function run(uploadDatabaseBundleDebugArtifact, uploadLogsDebugArtifact, printDebugLogs, repositoryNwo, featureEnablement, logger) {
    ...
    const uploadFailedSarifResults = await tryUploadSarifIfRunFailed(config, repositoryNwo, featureEnablement, logger);
