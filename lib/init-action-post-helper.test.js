...
(0, ava_1.default)("uploading failed SARIF run fails when workflow uses a complex upload input", async (t) => {
    ...
    t.is(result[0].upload_failed_run_error, "Could not get upload input to github/codeql-action/analyze since it contained an " +
...
(0, ava_1.default)("uploading failed SARIF run fails when workflow does not reference github/codeql-action", async (t) => {
    ...
    t.is(result[0].upload_failed_run_error, "Could not get upload input to github/codeql-action/analyze since the analyze job does not " +
