...
async function sendInitStatusReport(actionStatus, startedAt, config, toolsDownloadDurationMs, toolsFeatureFlagsValid, toolsSource, toolsVersion, logger) {
    ...
    if (config !== undefined) {
        ...
        if (toolsSource !== init_1.ToolsSource.Download) {
            await (0, actions_util_1.sendStatusReport)(statusReportWithConfig);
            return;
        }
        // Otherwise, we should append the extra two download-related telemetry fields.
        const toolsDownloadStatusReport = {
        ...
        await (0, actions_util_1.sendStatusReport)(toolsDownloadStatusReport);
        return;
    }
    else {
        if (toolsSource === init_1.ToolsSource.Download) {
        ...
        await (0, actions_util_1.sendStatusReport)(initBaseStatusReport);
    }
}
async function run() {
