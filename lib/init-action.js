...
async function sendInitStatusReport(actionStatus, startedAt, config, toolsDownloadDurationMs, toolsFeatureFlagsValid, toolsSource, toolsVersion, logger) {
    ...
    const statusReport = {
        ...
        trap_cache_download_size_bytes: Math.round(await (0, trap_caching_1.getTotalCacheSize)(config.trapCaches, logger)) || -1,
        trap_cache_download_duration_ms: Math.round(config.trapCacheDownloadTime) || -1,
    ...
    const downloadStatusReport = {
        ...statusReport,
        tools_download_duration_ms: toolsDownloadDurationMs ? toolsDownloadDurationMs : -1,
        tools_feature_flags_valid: toolsFeatureFlagsValid ? toolsFeatureFlagsValid : false, // Report invalid in case field is undefined.
    };
    await (0, actions_util_1.sendStatusReport)(downloadStatusReport);
}
async function run() {
