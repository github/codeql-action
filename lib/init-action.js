...
async function run() {
    ...
    try {
        ...
        if ("CODEQL_EXTRACTOR_GO_BUILD_TRACING" in process.env) {
            logger.warning(`Go is now analyzed as a compiled language by default, so the CODEQL_EXTRACTOR_GO_BUILD_TRACING environment variable is deprecated. We recommend that you remove this environment variable from your workflow. 
