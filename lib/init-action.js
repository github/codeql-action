...
async function run() {
    ...
    let configFile = (0, actions_util_1.getOptionalInput)("config-file");
    if (configFile === undefined) {
        const defaultLocation = path.join((0, util_1.getRequiredEnvParam)("GITHUB_WORKSPACE"), ".github", "codeql", "codeql.yml");
        if (fs.existsSync(defaultLocation)) {
            configFile = defaultLocation;
            logger.info(`Using default config file location: ${path.resolve(configFile)}`);
        }
    }
    try {
