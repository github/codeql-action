...
async function checkPacksForOverlayCompatibility(codeql, config, logger) {
    ...
    for (const language of config.languages) {
        const suitePath = path.join(config.dbLocation, language, "temp", "config-queries.qls");
        const packDirs = await codeql.resolveQueriesStartingPacks([suitePath]);
        for (const packDir of packDirs) {
            if (!checkPackForOverlayCompatibility(packDir, codeQlOverlayVersion, logger)) {
                return false;
            }
        }
    }
...
function checkPackForOverlayCompatibility(packDir, codeQlOverlayVersion, logger) {
    try {
        ...
        if (!fs.existsSync(packInfoPath)) {
            logger.warning(`The query pack at ${packDir} ` +
        ...
        if (typeof packOverlayVersion !== "number") {
            logger.warning(`The query pack at ${packDir} is not compatible with overlay analysis.`);
            return false;
        }
