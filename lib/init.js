...
async function installPythonDeps(codeql, logger) {
    ...
    try {
        ...
        logger.endGroup();
        logger.warning("We were unable to automatically install your python dependencies. Please make sure any necessary " +
            "dependencies are installed before calling the codeql-action/analyze step, and add a " +
            "'setup-python-dependencies: false' argument to this step to disable our automatic dependency " +
            "installation and avoid this warning.");
        return;
    }
    logger.endGroup();
}
