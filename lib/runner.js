...
program
    ...
    .option("--results-limit <resultsLimit>", "The maximum amount of results to be uploaded.")
    ...
    try {
        ...
        await upload_lib.uploadFromRunner(outputDir, (0, repository_1.parseRepositoryNwo)(cmd.repository), cmd.commit, parseRef(cmd.ref), cmd.category, sourceRoot, config.gitHubVersion, apiDetails, resultsLimit, logger);
    }
    catch (e) {
...
program
    ...
    try {
        ...
        const sourceRoot = cmd.checkoutPath || process.cwd();
        const resultsLimit = Number(cmd.resultsLimit) || Number.MAX_SAFE_INTEGER;
        await upload_lib.uploadFromRunner(cmd.sarifFile, (0, repository_1.parseRepositoryNwo)(cmd.repository), cmd.commit, parseRef(cmd.ref), cmd.category, sourceRoot, gitHubVersion, apiDetails, resultsLimit, logger);
    }
    catch (e) {
