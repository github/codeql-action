...
async function downloadCodeQL(codeqlURL, maybeCliVersion, apiDetails, variant, tempDir, logger) {
    ...
    const toolsDownloadDurationMs = perf_hooks_1.performance.now() - start;
...
async function setupCodeQLBundle(toolsInput, apiDetails, tempDir, variant, bypassToolcache, defaultCliVersion, logger) {
    ...
    switch (source.sourceType) {
        case "local":
            ...
            toolsSource = init_action_1.ToolsSource.Local;
            break;
        case "toolcache":
            codeqlFolder = source.codeqlFolder;
            logger.debug(`CodeQL found in cache ${codeqlFolder}`);
            toolsSource = init_action_1.ToolsSource.Toolcache;
            break;
        case "download": {
            ...
            toolsSource = init_action_1.ToolsSource.Download;
            break;
        }
        default:
