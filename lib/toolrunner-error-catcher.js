...
async function toolrunnerErrorCatcher(commandLine, args, matchers, options) {
    ...
    let cliReturnState;
    try {
        cliReturnState = await new toolrunner.ToolRunner(await safeWhich.safeWhich(commandLine), args, {
            ...options,
            listeners,
    ...
    if (matchers) {
        for (const matcher of matchers) {
            if (matcher.exitCode === cliReturnState ||
    ...
    if (typeof cliReturnState === "number") {
        ...
        if (options === null || options === void 0 ? void 0 : options.ignoreReturnCode) {
            ...
            throw new Error(`The process '${commandLine}' failed with exit code ${cliReturnState}`);
        }
    }
    else {
