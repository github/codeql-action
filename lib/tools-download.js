...
async function logDiskUsage(logger) {
    let stdout = "";
    let stderr = "";
    try {
...
async function verifyZstdIntegrity(archivedBundlePath, logger) {
    // Run `zstd -t` on the CodeQL bundle and log the results
    let stdout = "";
    let stderr = "";
    try {
        const zstdTestRunner = new toolrunner.ToolRunner("zstd", ["-t", archivedBundlePath], {
            silent: true,
            listeners: {
                stdout: (data) => {
                    stdout += data.toString();
        ...
        logger.warning(`Failed to verify the integrity of the CodeQL bundle using zstd: ${error}`);
    }
