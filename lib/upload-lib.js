...
async function uploadFromActions(sarifPath, gitHubVersion, apiDetails, resultsLimit, logger) {
    return await uploadFiles(getSarifFilePaths(sarifPath), (0, repository_1.parseRepositoryNwo)(util.getRequiredEnvParam("GITHUB_REPOSITORY")), await actionsUtil.getCommitOid(), await actionsUtil.getRef(), await actionsUtil.getAnalysisKey(), actionsUtil.getOptionalInput("category"), util.getRequiredEnvParam("GITHUB_WORKFLOW"), actionsUtil.getWorkflowRunID(), actionsUtil.getRequiredInput("checkout_path"), actionsUtil.getRequiredInput("matrix"), gitHubVersion, apiDetails, resultsLimit, logger);
}
exports.uploadFromActions = uploadFromActions;
// Uploads a single sarif file or a directory of sarif files
// depending on what the path happens to refer to.
// Returns true iff the upload occurred and succeeded
async function uploadFromRunner(sarifPath, repositoryNwo, commitOid, ref, category, sourceRoot, gitHubVersion, apiDetails, resultsLimit, logger) {
    return await uploadFiles(getSarifFilePaths(sarifPath), repositoryNwo, commitOid, ref, undefined, category, undefined, undefined, sourceRoot, undefined, gitHubVersion, apiDetails, resultsLimit, logger);
}
...
// Returns true iff the upload occurred and succeeded
async function uploadFiles(sarifFiles, repositoryNwo, commitOid, ref, analysisKey, category, analysisName, workflowRunID, sourceRoot, environment, gitHubVersion, apiDetails, resultsLimit, logger) {
...
function validateResultsLimit(sarif, resultsLimit) {
    var _a;
    let totalResults = 0;
    for (const run of sarif.runs) {
        totalResults += ((_a = run.results) === null || _a === void 0 ? void 0 : _a.length) || 0;
    }
    if (totalResults > resultsLimit) {
        throw new Error("Aborting upload: trying to upload " + totalResults + " but limit is set to " + resultsLimit);
    }
}
exports.validateResultsLimit = validateResultsLimit;
/**
