...
async function run() {
    ...
    try {
        ...
        const optResultLimit = actionsUtil.getOptionalInput("results_limit") || process.env["CODEQL_RESULTS_LIMIT"];
        const resultsLimit = (optResultLimit) ? Number(optResultLimit) : Number.MAX_SAFE_INTEGER;
        const uploadResult = await upload_lib.uploadFromActions(actionsUtil.getRequiredInput("sarif_file"), gitHubVersion, apiDetails, resultsLimit, (0, logging_1.getActionsLogger)());
