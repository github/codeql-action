...
var rspackResolver = require('rspack-resolver');
...
function normalizeOptions(options, cwd = process.cwd()) {
  ...
  options = __spreadProps$1(__spreadValues$1({
    ...
    project,
    tsconfig: {
      references: references != null ? references : "auto",
...
const resolve = (source, file, options, resolver) => {
  ...
  if (bunVersion || options.bun) {
    if (bunVersion ? module$1.isBuiltin(source) : isBunModule.isBunModule(source, bunVersion = "latest") || isBunModule.isSupportedNodeModule(source, bunVersion)) {
      log("matched bun core:", source);
      return { found: true, path: null };
    }
  } else if (module$1.isBuiltin(source)) {
  ...
  if (process.versions.pnp && source === "pnpapi") {
    return {
      found: true,
      path: module$1.findPnpApi(file).resolveToUnqualified(source, file, { considerBuiltins: false })
    };
  }
  ...
  if (!resolver) {
    ...
    if (!cached && !options.project) {
      resolverCache.set(cacheKey, cached = new rspackResolver.ResolverFactory(options));
    }
    resolver = cached;
  }
  options || (options = {});
  createResolver:
    if (!resolver) {
      ...
      for (const tsconfigPath of project) {
        ...
        resolver = new rspackResolver.ResolverFactory(options);
...
const createTypeScriptImportResolver = (options) => {
  ...
  const resolver = options.project ? null : new rspackResolver.ResolverFactory(options);
  return {
    interfaceVersion: 3,
