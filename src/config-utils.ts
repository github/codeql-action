...
export async function loadUserConfig(
  ...
  // Compute the path of the generated configuration file.
  const generatedConfigFile = userConfigFromActionPath(tempDir);

  // A helper function to decide if the given path is relative to the workspacePath.
  const isWorkspaceRelative = (fp: string) =>
    (fp + path.sep).startsWith(workspacePath + path.sep);

  // We must decide if `configFile` refers to a local or remote file. Using the old remote file
  // address format, an address referred to a remote file if it contained '@' somewhere. The
  // new format makes all components, except for the repo name optional, and (in theory) a
  // repo name could be something like `foo.yml`. That creates ambiguity over whether a
  // file is local or not.
  let localFile = false;

  if (configFile === generatedConfigFile) {
    // The generated file is always local.
    localFile = true;
  } else if (isLocal(configFile)) {
    // If the config file is not generated by the Action, it should be relative to the workspace.
    const localFilePath = path.resolve(workspacePath, configFile);

    // Determine whether the resulting path is relative to the workspace.
    ...
    if (
      ...
      if (isRelativePath(configFile) || !allowNewFormat) {
        if (workspaceRelative) {
          throw new ConfigurationError(
            errorMessages.getConfigFileDoesNotExistErrorMessage(localFilePath),
          );
        } else {
          throw new ConfigurationError(
  ...
  // Try to load the file according to whether it is local or not.
  if (localFile) {
